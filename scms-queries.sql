-- 1. Identifies customers whose outstanding balance exceeds their approved credit limit,
-- triggering a financial risk flag for the accounts receivable team to escalate.
SELECT company_name, outstanding_balance, credit_limit 
FROM customer 
WHERE outstanding_balance > credit_limit;

-- 2. Ranks all active, non-cancelled sales orders by lifetime revenue contribution per customer,
-- supporting strategic account prioritisation and key account management decisions.
...
-- 3. Quantifies the total on-hand inventory value per warehouse at standard cost,
-- enabling finance to report working capital tied up in stock by location.
...
-- 4. Flags all approved batches expiring within 60 days with their residual quantity and
-- potential write-off value, enabling proactive markdown or liquidation decisions.
...
-- 5. Surfaces all sales order line items where allocated quantity falls short of ordered quantity,
-- revealing active stockout or fulfilment gaps that require procurement or reallocation action.
...
-- 6. Lists all unpaid sales orders sorted by order age, providing the collections team
-- with a prioritised receivables worklist to reduce days sales outstanding (DSO).
...
-- 7. Highlights products whose current warehouse stock has fallen to or below the defined
-- reorder point, triggering replenishment review before stockout occurs.
...
-- 8. Detects approved-batch inventory with no movement in the last 90 days,
-- identifying dead stock that ties up warehouse space and working capital.
...
-- 9. Computes the variance between the unit price paid on each purchase order line and the
-- product standard cost, surfacing procurement overspend for supplier renegotiation.
...
-- 10. Lists all confirmed and allocated sales order lines still awaiting picking,
-- sorted by business priority level and order age to guide warehouse dispatch sequencing.
...
-- 11. Warns of approved-batch inventory expiring within 30 days with non-zero available quantity,
-- enabling FEFO dispatch teams to prioritise these batches in outbound order fulfilment.
...
-- 12. Traces all active in-transit shipments to their originating sales orders and customers,
-- enabling logistics teams to respond to customer delivery enquiries with live carrier data.
...
-- 13. Ranks the top 5 products by total units sold across all non-cancelled sales orders,
-- informing demand planning, promotional focus, and replenishment prioritisation.
...
-- 14. Computes the aggregate total amount committed on non-cancelled, non-fully-received
-- purchase orders per supplier, measuring active procurement exposure against supplier credit limits.
...
-- 15. Identifies products whose total available stock exists across multiple warehouses,
-- quantifying cross-location consolidation opportunities to reduce split-shipment overhead.
...
-- 16. Identifies all purchase orders that arrived after their expected delivery date, computing
-- the delay in days per supplier to support vendor scorecard and penalty clause evaluation.
...
-- 17. Measures the return rate per product by total returned quantity and return instance count,
-- flagging quality or fulfilment issues requiring supplier or process corrective action.
...
-- 18. Lists all return requests currently in 'requested' status awaiting approval,
-- providing the returns management team a pending action queue with customer context.
...
-- 19. Analyses active KYC-verified customers' credit utilisation and contracted payment terms
-- segmented by customer type, to refine credit policy and exposure management.
...
-- 20. Identifies all purchase orders in approved or partially-received state that have not yet
-- been fully fulfilled, sorted by expected delivery date to flag overdue inbounds.
...
-- 21. Retrieves the precise physical location of every inventory lot for a given product across
-- all warehouses, zones, and batches, supporting stock audit and pick-path planning.
...
-- 22. Analyses batches currently under quarantine hold with their on-hand quantity,
-- providing QA teams visibility into blocked stock pending inspection or disposal decisions.
...
-- 23. Aggregates active suppliers by tier, reporting average performance rating to support
-- strategic sourcing decisions and annual vendor rationalisation reviews.
...
-- 24. Traverses the full product category hierarchy using a recursive CTE, computing depth level
-- for each node to support category management, reporting rollups, and UI tree rendering.
...
-- 25. Lists all perishable products alongside their shelf life in days, supporting cold chain
-- planning, storage zone assignment, and FEFO dispatch policy configuration.
