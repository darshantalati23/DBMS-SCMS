-- ============================================================
--  SUPPLY CHAIN MANAGEMENT SYSTEM — PostgreSQL DDL
--  IT214 | Milestone 3 | Lab Group 05
--  Darshan Talati | Kavya Bhojwani | Shlok Patel | Aaryan Modi
-- ============================================================
...
-- ============================================================
--  SECTION 3 — STAFF
--  warehouse_id is nullable — central staff have no fixed
--  warehouse. NULL means they operate across all warehouses.
-- ============================================================
...
--  on_time_delivery_rate and quality_rejection_rate are updated
--  by the CLI app after PO closure and GRN inspection.
--  supplier_rating is DERIVED — never stored, computed in CLI.
-- ============================================================
...
-- parent_category_id is self-referential. NULL = root node.
-- DEFERRABLE allows inserting parent and child in same txn.
...
-- Pricing contract. One row per supplier-product-period.
-- Natural candidate key: (supplier_id, product_id, price_valid_from).
...
--  Partial key: zone_code (unique only within a warehouse).
--  Composite natural key enforced by UNIQUE(warehouse_id, zone_code).
-- ============================================================
...
-- State machine: draft→submitted→approved→partially_received
--                →fully_received / cancelled
-- State transitions enforced in CLI app.
-- GRN fields (grn_status, rejection_reason, received_by,
-- actual_delivery_date) are nullable — populated on goods receipt.
...
-- Partial key: product_id within a PO.
-- unit_price is a snapshot frozen at order time — never updated
-- even if the supplier contract price changes later.
-- line_total = ordered_quantity × unit_price (set by CLI on insert).
...
-- Physical lot received from a supplier via a PO line.
-- expiry_date is the FEFO sort key — CLI orders by this ASC.
-- quality_status starts as 'quarantine' until GRN inspection passes.
...
-- outstanding_balance is DERIVED — computed by CLI as
-- SUM(total_amount) WHERE payment_status != 'paid'.
-- Credit hold check enforced in CLI before creating sales order.
...
-- State machine: draft→confirmed→allocated→picking→packed
...
-- Partial key: product_id within an SO.
-- Four quantity columns track each fulfillment stage separately
-- enabling partial fulfillment visibility.
-- line_total = ordered_quantity × unit_price (set by CLI on insert).
...
-- Core ternary relationship. One row = exact stock position.
-- quantity_available = on_hand - reserved (DERIVED, CLI computes).
-- UNIQUE(batch_id, zone_id): one record per batch per zone.
...
-- FEFO audit trail. One SO_Item may draw from multiple
-- Inventory records — one row per (so_item, inventory) pair.
-- CLI handles FEFO logic and concurrency checks.
...
-- State machine: scheduled→loading→in_transit
--                →out_for_delivery→delivered / failed / returned
-- State transitions enforced in CLI app.
...
-- Partial key: so_item_id within a Shipment.
-- Links SO_Items to a Shipment for end-to-end traceability.
...
-- customer_id denormalized here for query convenience.
-- approved_by is nullable until a staff member approves it.
...
-- Partial key: so_item_id within a Return_Request.
-- restocked_to_inventory_id is nullable — only populated if
-- condition_on_arrival = 'resellable' after inspection.
...
-- ============================================================
--  SECTION 12 — TRIGGER-MANAGED TABLES
--  These exist as regular tables. In this static version,
--  the CLI app inserts into them directly instead of triggers.
-- ============================================================

-- ── 12.1 REORDER_ALERT ──────────────────────────────────────
-- CLI checks after every inventory update whether
-- quantity_on_hand < product.reorder_point and inserts here.
-- preferred_supplier_id is nullable if none is configured.
...
-- ── 12.2 AUDIT_LOG ──────────────────────────────────────────
-- CLI writes one row here after every INSERT/UPDATE/DELETE
-- on critical tables. BIGSERIAL for high-volume appends.
-- staff_id nullable for system-level operations.
...
-- CLI writes one row here after every stock change.
-- BIGSERIAL for high-volume appends.
